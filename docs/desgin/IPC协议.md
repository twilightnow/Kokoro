...
- 语音输入来源由配置决定，例如麦克风或系统内部来源
- STT 结果进入普通用户输入链路，不绕过聊天 API
- 原始音频保存必须显式配置
- IPC 不回传未经用户确认的原始音频内容

### Window / Display Preference
...
- 这些偏好属于用户运行时状态，不写回角色 manifest 或 personality
- 前端负责实际窗口行为，后端只保存和返回可治理偏好
- display 资源配置和用户显示偏好分离
- 窗口 resize 不应触发 display provider 配置重建
...
- `message`
- `recoverable`
- `auto_dismiss_ms`（可选，供前端控制短暂错误提示）
- `detail`（可选，脱敏）
